define (
  ...
  OSMInfobox.prototype = {
    getHTML: function(initiallayer, properties, element, language, map) {
      var renderdata = { 'language': language, 'fields': {}};
      var missing = '';
      ...
      for (var mykey in properties) {
        ...
        for (var layer of layers) {
          ...
          if (typeof keyvalues[layer] !== "undefined") {
            ...
            if (typeof keyvalues[layer][mykey]['values'] !== "undefined") {
              if (typeof keyvalues[layer][mykey]['values'][val] !== "undefined") {
                renderdata['fields'][mykey]['value'] = keyvalues[layer][mykey]['values'][val];
              } else {
                console.log("we found an undefined value, just dumping it raw");
                renderdata['fields'][mykey]['value'] = {'displayname': val}; 
              }
            } else {
              // If there is a name:language (or other :language) value, use that one.
              if (typeof properties[mykey + ':' + language] !== "undefined") {
                renderdata['fields'][mykey]['value'] = {'displayname': properties[mykey +':'+ language]}; 
              } else {
                renderdata['fields'][mykey]['value'] = {'displayname': val};
              }
            }
      ...
      sorted['fields'] = underscore.sortBy(renderdata['fields'], 'weight');
      var html = template(sorted);
      $(element).html(html + missing);

      // If CTRL is clicked, open in new tab
      $(element + " .icon").click({map: map}, function(event){
         if (key.ctrl) {
      ...
      $(element + " .label").click({map: map}, function(event){
        ...
        if (activetab != '') {
          map.sidebarcontrols['leftsidebar'].open(activetab+'pane');
        }
      ...
      $(element + " .wikipedia_white").click({map: map}, function(event){
        var url = '//en.m.wikipedia.org/wiki/'+this.attributes[2].nodeValue;
        $('#wikipediadoc').attr('src', url);
        event.data.map.sidebarcontrols['leftsidebar'].enable('wikipediadocpane');
        return false;
      });
      ...
      $(element + " .openstreetmap_bw").click({map: map}, function(event){
         var url = '//wiki.openstreetmap.org/w/index.php?title='+this.attributes[2].nodeValue+'&mobileaction=toggle_view_mobile';
         $('#osmwiki').attr('src', url);
              
        event.data.map.sidebarcontrols['leftsidebar'].enable('osmwikipane');
        return false;
      });
      $(element + " .wikidata_bw").click({map: map}, function(event){
        var url ='//m.wikidata.org/w/index.php?title='+this.attributes[2].nodeValue+'&mobileaction=toggle_view_mobile';
         $('#wikidata').attr('src', url);
        event.data.map.sidebarcontrols['leftsidebar'].enable('wikidatapane');
        return false;
      });
